...
test('treats chore commits as patch releases', async () => {
  assert.equal(await analyze(['chore: refresh generated assets']), 'patch')
})

test('treats ignored non-conventional commit subjects as patch releases', async () => {
  assert.equal(
    await analyze(['Refactor agreement subject resolution']),
    'patch',
  )
})

test('keeps feature commits as minor releases', async () => {
  assert.equal(await analyze(['feat: add shared morphology analysis']), 'minor')
})

test('keeps breaking changes as major releases', async () => {
  assert.equal(
    await analyze([
      'feat!: change public API\n\nBREAKING CHANGE: the API now requires a config object',
    ]),
    'major',
  )
})
